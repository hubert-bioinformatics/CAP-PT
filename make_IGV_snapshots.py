...
def make_chrom_region_list(region_file, nf4_mode= True):
    '''
    ...
    with open(region_file) as f:
        for line in f:
            if nf4_mode == True:
                if len(line.split('\t')) >= 4:
                    chrom, start, stop, order = line.split()[0:4]
                    region_list.append((chrom, start, stop, order))
            else:
                if len(line.split()) >= 3:
...
def make_snapshot_filename(region, height, suffix = None):
    ...
    '''
    if len(region) >= 4:
        chrom, start, stop, order = region[0:4]
        return('{}_{}_{}_{}_h{}.png'.format(order, chrom, start, stop, height))
...
def run_IGV_script(igv_script, igv_jar, memMB):
    ...
    print('\tOpen Xvfb port found on: {}'.format(x_serv_port))
    ...
    print('\tIGV command is: {}'.format(igv_command))
    # get current time; command can take a while to finish
    startTime = datetime.datetime.now()
    print("\tCurrent time is: {}".format(startTime))
    # run the IGV command
    print("\tRunning the IGV command...")
    subprocess_cmd(igv_command)
    elapsed_time = datetime.datetime.now() - startTime
    print("\tIGV finished; elapsed time is: {}".format(elapsed_time))



def main(input_files, region_file = 'regions.bed', genome = 'hg19', image_height = '2000', outdir = 'IGV_Snapshots', igv_jar_bin = "bin/IGV_2.3.81/igv.jar", igv_mem = "20000", no_snap = False, suffix = None, nf4_mode = False, onlysnap = False):
    '''
    ...
    print('\tReference genome: {}'.format(genome))
    print('\tRegion file: {}'.format(region_file))
    for file in input_files:
...
def run():
    ...
    parser.add_argument("-ht", default = '2000', type = str, dest = 'image_height', metavar = 'image height', help="Height for the IGV tracks")
    ...
    parser.add_argument("-nf4", default = True, action='store_true', dest = 'nf4_mode', help="'Name field 4' mode; uses the value in the fourth field of the regions file as the filename for each region snapshot")
